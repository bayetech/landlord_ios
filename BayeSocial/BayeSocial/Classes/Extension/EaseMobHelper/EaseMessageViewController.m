...
@property (nonatomic,strong) NSMutableDictionary *atMessageList;
@property (nonatomic,strong) NSMutableDictionary *atMeUserList;
                           ...
                           conversationType:(EMConversationType)conversationType
{
    ...
    if (self) {
        ...
        self.groupChat = [conversationChatter isNumberValue];;
...
- (NSMutableDictionary *)atMessageList {
    if (!_atMessageList) {
        _atMessageList = [NSMutableDictionary dictionary];
    }
    return _atMessageList;
}

- (NSMutableDictionary *)atMeUserList {
    if (!_atMeUserList) {
        _atMeUserList = [NSMutableDictionary dictionary];
    }
    return _atMeUserList;
}
...
- (void)viewDidLoad {
    ...
    EMChatToolbarType barType = !self.isGoupChat == EMConversationTypeChat ? EMChatToolbarTypeChat : EMChatToolbarTypeGroup;
...
//    UIImage *userBubble                 = [UIImage imageNamed:@"bubbleUser"];
//    [[EaseBaseMessageCell appearance] setSendBubbleBackgroundImage:[userBubble stretchableImageWithLeftCapWidth:10.0f topCapHeight:userBubble.size.height *0.8f]];
    
    UIImage *otherBubble                 = [UIImage imageNamed:@"bubbleOther"];
    [[EaseBaseMessageCell appearance] setRecvBubbleBackgroundImage:[otherBubble stretchableImageWithLeftCapWidth:otherBubble.size.width *0.5f topCapHeight:otherBubble.size.height *0.7f]];
    ...
    self.inputTextField  = self.chatToolbar.inputTextView;
...
/**
 群组成员发生改变的通知
 */
- (void) groupMembersChange:(NSNotification *)noti {
    
    NSString *userId    = noti.object;
    if (!userId) return;
    __weak typeof(self) weakself = self;
    [[AppDelegate appDelegate] reqeustCustomerUserList:userId completion:^(NSArray<BKCustomersContact *> *customers) {
        __strong typeof(weakself)strongself = weakself;
        BKCustomersContact *customerUser = [customers lastObject];
        if (customerUser) {
            strongself.groupMembers[customerUser.uid] = customerUser;
        }
    }];
    
}
#pragma mark 顶部@消息提示 view
/**
  顶部提醒你未读消息的 view
 */
- (void) addTopView {
    
    self.topView                        = [[UIView alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width-100.0f, 15.0f, 120.0f, 35.0f)];
    self.topView.backgroundColor        = [UIColor whiteColor];
    self.topView.layer.cornerRadius     = 17.5;
    self.topView.layer.masksToBounds    = YES;
    self.topView.layer.borderColor      = [UIColor RGBColor:201.0f green:201.0f blue:201.0f].CGColor;
    self.topView.layer.borderWidth      = 0.5f;
    [self.view addSubview:self.topView];
    self.tableView.backgroundColor      = [UIColor RGBColor:243.0f green:243.0f blue:243.0f];
    
    // 头像
    UIImageView *headImageView          = [[UIImageView alloc] init];
    [headImageView setImage:[UIImage imageNamed:@"user_unregister"]];
    headImageView.frame                 = CGRectMake(3.0f, 3.0f, 29.0f,  29.0f);
    headImageView.layer.cornerRadius    = 14.5f;
    headImageView.layer.masksToBounds   = YES;
    [self.topView addSubview:headImageView];
    _atUserImageView                    = headImageView;
    // @我 label
    UILabel *label                      = [[UILabel alloc] init];
    label.text                          = @"@我";
    label.textColor                     = [UIColor RGBColor:243.0f green:87.0f blue:0.0f];

    label.font                          = [UIFont systemFontOfSize:15.0];
    label.textAlignment                 = NSTextAlignmentCenter;
    [self.topView addSubview:label];
    label.frame                         = CGRectMake(headImageView.right, 0.0f, 50.0f, 35.0f);
    
    [self.topView addTarget:self action:@selector(tapTopView)];
    self.topView.hidden                 = YES;
    
}

/**
 点击顶部提醒 view 查看@我的人消息
 */
- (void)tapTopView {
    
    NSIndexPath *indexPath              = self.atMessageList.allValues.firstObject;
    if (!indexPath) return;
    [self.tableView scrollToRowAtIndexPath:indexPath atScrollPosition:UITableViewScrollPositionTop animated:YES];
    [self.atMessageList removeAllObjects];
    [self.atMeUserList removeAllObjects];
    [self.topView removeFromSuperview];
    self.topView                        = nil;
    self.atMeUserList                   = nil;
    self.atMessageList                  = nil;
...
/**
 展示未读消息
 */
- (void)showUnreadAtMessage {
    
    if (!self.topView) return;
    if (self.atMessageList.count == 0) return;
    __weak typeof(self) weakself = self;
    dispatch_async(dispatch_get_main_queue(), ^{
        __strong typeof(weakself)strongself = weakself;
        NSArray *indexPaths =  [strongself.tableView indexPathsForVisibleRows];
        for (NSIndexPath *visibleIndexPath in indexPaths) {
            NSString *key               = [NSString stringWithFormat:@"%ld",visibleIndexPath.row];
            NSIndexPath *atIndexPath    = self.atMessageList[key];
            if (atIndexPath) return;
        }
        strongself.topView.hidden    = NO;
        NSString *userId             = [self.atMeUserList.allValues firstObject];
        BKCustomersContact *atMeUser = [self.groupMembers objectForKey:userId];
        if (!atMeUser || !atMeUser.avatar) return;
        [_atUserImageView sd_setImageWithURL:[NSURL URLWithString:atMeUser.avatar] placeholderImage:[UIImage imageNamed:@"user_unregister"]];
    });
...
- (NSMutableDictionary<NSString *,BKCustomersContact *> *)groupMembers {
    if (!_groupMembers) {
        _groupMembers = [NSMutableDictionary dictionary];
    }
...
/**
 获取群组成员的资料
 */
- (void)requestGroupMembersInfo {
    
    __weak typeof(self) weakself = self;

    [[AppDelegate appDelegate] requestGroupMembersBy:self.conversation.conversationId completion:^(NSArray<BKCustomersContact *> *groupMembers) {
        __strong typeof(weakself)strongself = weakself;
        [strongself loadCustomerUser:groupMembers];
        [strongself showUnreadAtMessage];
    }];
...
/**
 获取群成员资料
 */
- (void) loadCustomerUser:(NSArray *)customers {
    
    __weak typeof(self) weakself = self;
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        __strong typeof(weakself)strongself = weakself;
        for (BKCustomersContact *contact in customers) {
            strongself.groupMembers[contact.uid] = contact;
        }
        dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    messageType:(EMMessageBodyType)messageType
{
    ...
    if (!_transmitMenuItem) {
        _transmitMenuItem = [[UIMenuItem alloc] initWithTitle:@"转发" action:@selector(transmitMsgAction:)];
    }
    ...
    void (^refresh)(NSArray *messages) = ^(NSArray *messages) {
        dispatch_async(_messageQueue, ^{
            ...
            for (EMMessage *message in messages)
            {
                ...
                // 如果有拓展消息的话 就解析拓展消息中的 @我的信息 然后放到数组里,点击查看@我信息时跳动这个消息所在的索引未知
                NSArray *em_at_list                 = ext[@"em_at_list"];
                if (ext && em_at_list) {
                    if ([em_at_list containsObject:[[BKAuthorizationToken shared] easemob_username]]) {
                        NSIndexPath *indexPath = [NSIndexPath indexPathForRow:index inSection:0];
                        NSString *key          = [NSString stringWithFormat:@"%ld",(long)indexPath.row];
                        strongself.atMessageList[key] = indexPath;
                        [strongself.atMeUserList setObject:message.from forKey:key];
                    }
                    message.ext = nil;
                    [strongself.conversation updateMessageChange:message error:nil];
                }
                
                [strongself _downloadMessageAttachments:message];
                index ++;
            }
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.atMessageList == 0) return;
    NSIndexPath *atIndexPath    = self.atMessageList[[NSString stringWithFormat:@"%ld",(long)indexPath.row]];
    if (!atIndexPath)           return;
    [self.topView removeFromSuperview];
    self.topView                = nil;
    [self.atMessageList removeAllObjects];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        // 红包消息
        if (model.message.ext[@"send_red_packets"]) {
            
            UITableViewCell *redPacketCell  = [self.dataSource messageViewController:tableView cellForRedPacketMessageModel:model];
            redPacketCell.tag               = indexPath.row;
            
            // 头像
            UIImageView *avatarView         = [redPacketCell valueForKey:@"avatarImageView"];
            avatarView.tag                  = indexPath.row;
            [avatarView addGestureRecognizer:[[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(longPressAvatar:)]];
            
            return  redPacketCell;
        } else if (model.message.ext[@"open_red_packets"]) { // 领取红包的 cell

            UITableViewCell *redPacketCell      = [self.dataSource messageViewController:tableView cellForDidOpenRedPacketMessageModel:model];
            redPacketCell.tag                   = indexPath.row;
            return  redPacketCell;
        } else if (model.message.ext[@"userCard"]) { // 用户名片功能
            
            UITableViewCell *userCardCell       = [self.dataSource messageViewController:tableView cellForUserCardWithMessageModel:model];
            userCardCell.tag                   = indexPath.row;
            return  userCardCell;
        }
        ...
        sendCell.model                  = model;
        sendCell.avatarView.tag         = indexPath.row;
        [sendCell.avatarView addGestureRecognizer:[[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(longPressAvatar:)]];
        [self setupUserInfo:model cell:sendCell];
...
/**
 长按头像点击
 */
- (void) longPressAvatar:(UILongPressGestureRecognizer *)longGes {
    
    UIImageView *avatar             = (UIImageView *)longGes.view;
    id<IMessageModel> model         = [self.dataArray objectAtIndex:avatar.tag];
    avatar.userInteractionEnabled   = longGes.state != UIGestureRecognizerStateBegan;

    if (longGes.state == UIGestureRecognizerStateBegan) {
        [self didLongPressAvatar:model.message.from];
    }
...
/**
 设置头像和昵称信息
 */
- (void)setupUserInfo:(id<IMessageModel>)model cell:(EaseBaseMessageCell *)baseCell {
    
    BKCustomersContact *user    = [self getCustomerModel:model];
    NSString *username          = user.name;
    NSString *avatar            = user.avatar;
    NSString *userId            = user.uid;
    
    baseCell.nameLabel.text     = username;

    [baseCell.avatarView sd_setImageWithURL:[NSURL URLWithString:avatar] placeholderImage:[UIImage imageNamed:@"user_unregister"]];
    
    baseCell.userId             = userId;
...
- (BKCustomersContact *_Nonnull) getCustomerModel:(id<IMessageModel>)model {
    ...
    if (model.isSender) {
        return self.userSelf;
    } else {
        if (self.isGoupChat) {
            BKCustomersContact *otherUser = self.groupMembers[model.message.from];
            if (!otherUser) {
                otherUser = [[BKRealmManager shared] queryCustomuserUsersIntable:model.message.from];
            }
            self.userOther = otherUser;
        }
        return self.userOther;
    }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        // 红包消息高度
        NSDictionary *redPacketExt       = model.message.ext[@"send_red_packets"];
        if (redPacketExt) {
            return [self.delegate messageViewController:self heightForRedPacketMessageModel:model];
        } else if (model.message.ext[@"open_red_packets"]) { // 领取红包的 cell
            return 35.0f;
        } else if (model.message.ext[@"userCard"]) {
            return [self.delegate messageViewController:self heightForUserCardMessageModel:model];
        }
...
- (void)avatarViewSelcted:(id<IMessageModel>)model {
    ...
    [self didSelectUserAvatar:model.message.from];
    _scrollToBottomWhenAppear = NO;
}
...
/**
 ...
 @param userId 用户 id
 */
- (void)didSelectUserAvatar:(NSString *)userId {
    
    if ([self.delegate respondsToSelector:@selector(messageViewController:didSelectAvatarUserid:)]) {
        [self.delegate messageViewController:self didSelectAvatarUserid:userId];
    }
...
- (BOOL)didInputAtInLocation:(NSUInteger)location
{
    ...
    if ([self.delegate respondsToSelector:@selector(messageViewController:selectAtTarget:)] && self.isGoupChat) {
        location += 1;
        __weak typeof(self) weakSelf = self;
...
/**
 去商城 */
- (void)moreViewStoreAction:(EaseChatBarMoreView *)moreView {
    
    [self.chatToolbar endEditing:YES];
    BKStoreViewController *storeViewController = [[BKStoreViewController alloc] init];
...
/**
 视频通话
 */
- (void)moreViewVideoChatAction:(EaseChatBarMoreView *)moreView {
    [self.chatToolbar endEditing:YES];
}
...
- (void)didReceiveMessages:(NSArray *)aMessages
{
    for (EMMessage *message in aMessages) {
        if ([self.conversation.conversationId isEqualToString:message.conversationId]) {
            ...
            // 播放声音和震动
            [[EMIMHelper shared] playSoundAndVibration];
        }
    }
}
...
- (NSArray *)formatMessages:(NSArray *)messages
{
    ...
    for (EMMessage *message in messages) {
        ...
        NSDictionary *ext                       = [model.message.ext replaceJsonToObject];
        model.message.ext                       = ext;
        [self.conversation updateMessageChange:model.message error:nil];
        // 如果领取红包的消息与我无关,那么就移除这条消息
        if (ext[@"open_red_packets"]) {
            NSDictionary *open_red_packets      = ext[@"open_red_packets"];
            NSNumber *canShowRedPacketMsg       = (NSNumber *)[[EMIMHelper shared] showRedPacketMessage:open_red_packets][@"showMessage"];
            if (!canShowRedPacketMsg.boolValue) {
...
- (void)sendTextMessage:(NSString *)text
{
    ...
    if (self.isGoupChat) {
        ...
        // 发送@消息的时候附带个人资料
        //WARNING
//        NSDictionary *dict          = self.userSelf.mj_keyValues;
//        ext[@"customer"]            = dict;
    }
