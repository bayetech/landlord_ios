...
func getProperties(forType type: Any.Type) -> [Property.Description]? {
    if let nominalType = Metadata.Struct(fromType: type) {
        return fetchProperties(nominalType: nominalType)
    } else if let nominalType = Metadata.Class(fromType: type) {
        return nominalType.properties()
    } else if let nominalType = Metadata.ObjcClassWrapper(fromType: type),
